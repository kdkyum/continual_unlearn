...
def generate_sparsity_values():
    ...
    # Add the additional higher sparsity values
    # additional_values = np.array([0.2, 0.3, 0.4, 0.5])
    # Combine and return all values
    # return np.append(log_values, additional_values)
    # return np.array([0.0001, 0.0003, 0.001, 0.01, 0.1])#log_values
...
def generate_slurm_script(args, dataset, model, method, lr, sparsity=None, layer_wise=True):
    ...
    if method == "RL" and mask_dir is None:
        mask_dir = f"{args.save_dir}/{dataset}/{model}/masks"
    if method == "RL":
        os.makedirs(mask_dir, exist_ok=True)
    
    script_path = f"{args.output_dir}/{job_id}.sh"
    
    # Determine the last class
    last_class = get_last_class(dataset)
    
    # Prepare the save directory for this specific run
    save_path_base = f"{args.save_dir}/{dataset}/{model}/{method}/lr{lr:.6f}"
    if sparsity is not None:
...
def main():
    ...
    for dataset, model, method in itertools.product(datasets, models, methods):
        if method != "synaptag":
            ...
            for lr, sparsity, layer_wise_flag in itertools.product(learning_rates, sparsity_values, [True, False]):
