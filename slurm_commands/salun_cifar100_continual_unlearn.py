...
"""
Script to generate a Slurm job for running the SalUn algorithm on CIFAR-100
...
def parse_args():
    parser = argparse.ArgumentParser(description='Generate Slurm script for cumulative SalUn unlearning on CIFAR-100')
    parser.add_argument('--save_dir', type=str, default="checkpoints/cifar100_salun_continual_unlearn",
                        help='Directory to save the SalUn models')
    parser.add_argument('--mask_dir', type=str, default="checkpoints/cifar100_salun_continual_unlearn/masks",
    ...
    parser.add_argument('--job_name', type=str, default="cifar100_salun_continual_unlearn",
...
def generate_slurm_script(args):
    ...
    script_path = f"{args.output_dir}/cifar100_salun_cumulative_unlearn_{timestamp}.sh"
...
for END_CLASS in $(seq $INCREMENT $INCREMENT $MAX_CLASS); do
    ...
    # Step 1: Generate the saliency map for the new classes
    MASK_SUBDIR="${MASK_DIR}/0-${END_CLASS}"
    mkdir -p ${MASK_SUBDIR}
    MASK_PATH="${MASK_SUBDIR}/with_${THRESHOLD}.pt"
    
    # If there are multiple new classes, choose the first one for the mask 
    # (SalUn generates the mask for a single class)
    MASK_CLASS=$(echo ${NEW_CLASSES} | awk '{print $1}')
    echo "Generating saliency map using class: ${MASK_CLASS}"
    
    MASK_COMMAND="python generate_mask.py --save_dir ${MASK_SUBDIR} \\
        --class_to_replace ${MASK_CLASS} --unlearn_epochs 1 \\
...
def main():
    ...
    print(f"Generating Slurm job for cumulative SalUn unlearning of CIFAR-100 classes in increments of {args.increment} up to class {args.max_class}...")
