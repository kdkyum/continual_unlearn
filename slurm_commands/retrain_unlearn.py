...
"""
Script to generate a Slurm job for retraining ResNet-18 on CIFAR-10
to unlearn classes 0-7 using the retrain method.
...
def parse_args():
    ...
    parser.add_argument('--save_dir', type=str, default="checkpoints/retrain_model",
    ...
    parser.add_argument('--job_name', type=str, default="retrain_unlearn",
...
def generate_slurm_script(args):
    ...
    script_path = f"{args.output_dir}/retrain_unlearn_{timestamp}.sh"
...
#
# Run as array job for each forget class combination (0-0 to 0-7)
#SBATCH --array=0-7
#
#SBATCH --ntasks=1
...
# Set the target classes to unlearn based on array task ID
...
echo "Starting retraining unlearning for classes ${{CLASS_IDS}}..."
echo "Saving model to ${{SAVE_DIR}}"

python main_forget.py --save_dir ${{SAVE_DIR}} \\
    --unlearn retrain --class_to_replace ${{CLASS_IDS}} --unlearn_epochs ${{EPOCHS}} \\
    --unlearn_lr ${{LR}} --data ${{DATA_PATH}}

echo "Retrain unlearning complete for classes ${{CLASS_IDS}}!"
"""
...
def main():
    ...
    print(f"Generating Slurm array job for parallel retraining to unlearn classes 0 through 0-7...")
